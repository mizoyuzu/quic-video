...
IMAGでは画質だけでなく動きの連続性、カメラパン時の破綻、キーフレーム待ち時間が重要です。最初の実機はiPhone 14 Proの背面カメラとし、480p/720p/1080pと30fps/60fpsを設定で選べるようにします。100ms未満はiPhone撮影からOBS表示までの最終目標ですが、カメラ、エンコード、Wi-Fi、relay、FFmpeg、OBS表示の合計値なので、各段階を個別に記録します。
